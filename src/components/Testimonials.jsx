...
export default function Testimonials() {
  ...
  return (
    <section ref={sectionRef} id="depoimentos" className="py-16 lg:py-24 bg-[#FAF6E4] relative overflow-hidden">
      <BackgroundCircle size={360} color="#FEF9C3" opacity={0.4} className="-top-16 right-1/3 z-0" />
      <DottedPattern rows={5} cols={6} color="#EAB308" opacity={0.12} className="absolute bottom-6 left-10 z-0 hidden sm:block" />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-12 relative z-10">
        ...
        <div className="text-center mb-10">
          ...
          {/* Authority Badges */}
          <div className="mt-8 inline-flex items-center justify-center gap-6 sm:gap-10 bg-white/90 backdrop-blur-md px-6 py-3.5 rounded-full border border-[#FDE68A] shadow-sm">
            <div className="hidden sm:flex items-center gap-1.5 text-xs font-bold text-[#1C1400] border-r border-[#FDE68A] pr-4">
            ...
            <div className="flex items-center gap-4 sm:gap-6">
              {actionBadges.map((b, idx) => (
                <div key={idx} className="flex items-center gap-2 group">
                  <div className="w-11 h-11 sm:w-12 sm:h-12 rounded-full p-0.5 bg-gradient-to-tr from-[#EAB308] to-blue-300 shadow-xs group-hover:scale-105 transition-transform shrink-0 overflow-hidden">
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex md:hidden items-center gap-2">
            <button onClick={() => scroll('left')} className="w-9 h-9 rounded-full bg-white border border-[#FDE68A] text-[#1C1400] flex items-center justify-center shadow-xs active:scale-95 transition-all" aria-label="Anterior">
              <ChevronLeft size={18} />
            </button>
            <button onClick={() => scroll('right')} className="w-9 h-9 rounded-full bg-[#EAB308] text-[#1C1400] flex items-center justify-center shadow-md shadow-yellow-500/20 active:scale-95 transition-all" aria-label="Próximo">
        ...
        <div ref={scrollRef} className="flex md:grid md:grid-cols-3 overflow-x-auto md:overflow-visible snap-x snap-mandatory gap-6 pb-4 md:pb-0 scrollbar-none -mx-4 px-4 sm:mx-0 sm:px-0 relative z-10">
          {reviews.map((r, i) => (
            <div
              key={i}
              className="testimonial-card group w-[86vw] max-w-[360px] md:w-auto md:max-w-none snap-center shrink-0 rounded-3xl p-6 sm:p-8 border border-[#FDE68A]/80 shadow-md flex flex-col justify-between transition-all duration-300 cursor-default relative overflow-hidden
                bg-white hover:bg-white/60 hover:backdrop-blur-md hover:border-[#EAB308]/40 hover:shadow-xl hover:-translate-y-1"
            >
              ...
              <div className="relative z-10">
                <div className="flex items-center justify-between mb-4">
                  <Quote className="text-[#EAB308] w-8 h-8 opacity-40" />
                ...
                <p className="text-[#44330A] text-xs sm:text-sm leading-relaxed italic mb-6">"{r.text}"</p>
              </div>
              ...
              <div className="pt-4 border-t border-gray-100 flex items-center gap-3 relative z-10">
                ...
                <div>
                  ...
                  <span className="text-[11px] text-[#EAB308] font-semibold">{r.role}</span>
                </div>
              </div>
            </div>
          ))}
        ...
        <div className="flex md:hidden justify-center gap-2 mt-5">
          {reviews.map((_, i) => (
            ...
            >
              <div className={`rounded-full transition-all duration-300 ${i === activeIdx ? 'w-6 h-2.5 bg-[#EAB308]' : 'w-2.5 h-2.5 bg-[#FDE68A]'}`} />
            </button>
          ))}
        </div>
